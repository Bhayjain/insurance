 ...
 class Content extends Component {
    constructor( props ) {
        ...
        this.state = {
            ...
            loading:false,
            esic_control : Cookies.get("genral_report_control")

        };
        this.esic_report(this.state.sort_date)
    }

    esic_report = (sort_date)=>  {
     ...
     console.log("params",params);
         const res = fetch(settings.api_url + "api/payroll/esic_report", {
             method: 'POST',
    ...
    export_esic_report = ()=>  {
     ...
     console.log("Export params",params);
         const res = fetch(settings.api_url + "api/payroll/export_esic_report", {
             method: 'POST',
             body: JSON.stringify(params),
             headers: {
                 "Content-type": "application/json; charset=UTF-8",
                 ...
                 <PageTitle className="payroll_new">
                      <div className="row">
                         <div className="col-lg-8 col-md-4 col-sm-12">
                             <h1 className="heading_top">ESIC Report</h1>
                         </div>
                         <div className="col-lg-4 col-md-4 col-sm-12 disPlayNewData">
                         <DatePicker
                        ...
                        />
                        <div>
                        <Button  disabled={this.state.esic_control == "false" ? true : false} style={{marginLeft : "10px" }} onClick={()=>this.export_esic_report()}>Export{this.state.loading ? (
                             <Spinner />
                         ) : ''}</Button>
                        </div>
                      </div>
                                   ...
                                   <Table striped>
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col" className="border_td" style={{textAlign:"end"}}>ESIC Amount</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {this.state.esic_array.map((value,index)=>{
                                                return(
                                            <tr key={index}>
                                                <th scope="row" className="border_td">
                                                    <div>{value.emp_name}</div>
                                                </th>
                                                <td className="border_td" style={{textAlign:"end"}}>&#x20b9;{(value.esic_amount).toFixed(2)}</td>
                                            </tr>
                                                )
                                            })}
