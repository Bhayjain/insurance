 ...
 class Content extends Component {
     ...
     fetch_all_daily_sales_report = (search_sm,startDate,endDate,pageNumber)=>  {
        ...
        console.log("Reporttt",params);
         const { settings } = this.props;
          const res = fetch(settings.api_url + "fetch_pending_payout_report", {
              method: 'POST',
     ...
     daily_sales_report = ()=>  {
        ...
        //console.log("Export Params",params);
          const res = fetch(settings.api_url + "export_pending_payout_report", {
              method: 'POST',
              body: JSON.stringify(params),
              headers: {
                  "Content-type": "application/json; charset=UTF-8",
              ...
              .then(json => {
                  //console.log("fetch Export Pending Payout ****", json)
                  var data = json;
                  if (data.status == true) {
             ...
             <Fragment>
              ...
              <div className="mycalendar" style={{display: this.state.no_data=="none" ? "block" :"none",height:my_height - 70}}>
                 ...
                 <Table striped>
                         <thead>
                             <tr>
                                 <th scope="col" className="table_head_new">Employee Names</th>
                                 <th scope="col" className="table_head_new" style={{textAlign:"end"}}>No Of Policy</th>
                                 <th scope="col" className="table_head_new" style={{textAlign:"end"}}>Payout Amount</th>
                             </tr>
                         </thead>
                         <tbody>
                             {this.state.daily_sales_array.map((value,index)=>{
                                 ...
                                 return(
                                     ...
                                     <td className="table_sub_head" style={{textAlign:"end"}}>&#x20b9; {value.payout_amount}</td>
                                 </tr>
                                 )
                             })}
                              <tr >
                                     ...
                                     <td className="table_sub_head total_new" style={{textAlign:"end"}}>&#x20b9; {this.state.total_payout_amount}</td>
                                 </tr>
