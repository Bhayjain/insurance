 ...
 class Content extends Component {
    constructor( props ) {
        ...
        this.state = {
            ...
            loading:false,
            payroll_control : Cookies.get("genral_report_control")

        };
        this.emp_payroll_report(this.state.sort_date)
    }

    emp_payroll_report = (sort_date)=>  {
     ...
     console.log("params",params);
         const res = fetch(settings.api_url + "api/payroll/emp_payroll_report", {
             method: 'POST',
             ...
             .then(json => {
                 console.log("Fetch ESIC ***************", json)
                 var data = json;
                 if (data.status == true) {
                     this.setState({
                      payroll_array: data.data,
                      isLoading:"none",
                      no_data_message:"none",
                     });
                 }
    ...
    export_emp_payroll_report = ()=>  {
     ...
     console.log("Export params",params);
        //  const res = fetch(settings.api_url + "api/payroll/export_emp_payroll_report", {
        //      method: 'POST',
        //      body: JSON.stringify(params),
        //      headers: {
        //          "Content-type": "application/json; charset=UTF-8",
                 ...
                 <PageTitle className="payroll_new">
                      <div className="row">
                         <div className="col-lg-8 col-md-4 col-sm-12">
                             <h1 className="heading_top">Payroll Report</h1>
                         </div>
                      ...
                      <div className="col-lg-1 col-md-4 col-sm-12" style={{paddingLeft:"0px", textAlign:"end"}}>
                            <Button disabled={this.state.payroll_control == "false" ? true : false} onClick={()=>this.export_emp_payroll_report()}>Export{this.state.loading ? (
                              <Spinner />
                              ) : ''}</Button>
                         </div>
                                   ...
                                   <Table striped>
                                        <thead>
                                            ...
                                            <th scope="col" className="border_none">Employee Name</th>
                                                ...
                                                <th scope="col" className="border_none" style={{textAlign:"end"}}>Salary Paid</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {this.state.payroll_array.map((value,index)=>{
                                                console.log(value.payable_amount);
                                                return(
                                                    <tr key={index}>
                                                    ...
                                                    <div className="net_salary_hover">
                                                        &#x20b9;{(value.pending_dues).toFixed(2)}
                                                        {/* <Icon style={{display : value.pending_dues_breakdown == "" ? "none" : "inline-flex"}} className="alert_icon" name="alert-circle" /> */}
                                                ...
                                                <td className="border_td" style={{textAlign:"end"}}>
                                                     <div className="net_salary_hover">
                                                    {value.total_additional == null ? "" : <span> &#x20b9; {value.total_additional == null ? "" :(value.total_additional).toFixed(2) }</span>}

                                                        <Icon style={{display : value.additional_breakdown.length == 0 ? "none" : "inline-flex"}} className="alert_icon" name="alert-circle" />
