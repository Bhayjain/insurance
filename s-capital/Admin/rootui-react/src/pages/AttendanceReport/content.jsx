...
import Cookies from 'js-cookie';
 ...
 class Content extends Component {
    constructor( props ) {
        ...
        this.attendance_report(this.state.sort_date)
    }

    attendance_report = (sort_date)=>  {
     ...
     console.log("params",params);
         const res = fetch(settings.api_url + "api/payroll/attendance_report", {
             method: 'POST',
    ...
    export_attendance_report = ()=>  {
     ...
     console.log("Export params",params);
         const res = fetch(settings.api_url + "api/payroll/export_attendance_report", {
             method: 'POST',
             body: JSON.stringify(params),
             headers: {
                 "Content-type": "application/json; charset=UTF-8",
                 ...
                 <PageTitle className="payroll_new">
                      <div className="row">
                         <div className="col-lg-8 col-md-4 col-sm-12">
                             <h1 className="heading_top">Attendance Report</h1>
                         </div>
                      ...
                      <div className="col-lg-1 col-md-4 col-sm-12" style={{paddingLeft:"0px", textAlign:"end"}}>
                            <Button disabled={this.state.payroll_control == "false" ? true : false} onClick={()=>this.export_attendance_report()}>Export{this.state.loading ? (
                                                                <Spinner />
                                                            ) : ''}</Button>
                         </div>
                                   ...
                                   <Table striped>
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col" className="border_td" >Present</th>
                                                <th scope="col" className="border_td" >Absent</th>
                                                <th scope="col" className="border_td" >Half Day</th>
                                                <th scope="col" className="border_td" >Paid Leave</th>
                                                <th scope="col" className="border_td" >Unmarked</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {this.state.attendance_array.map((value,index)=>{
                                                return(
                                            <tr key={index}>
                                                <th scope="row" className="border_td">
                                                    <div>{value.emp_name}</div>
                                                </th>
                                                <td className="border_td">{value.present_count}</td>
                                                <td className="border_td">{value.absent_count}</td>
                                                <td className="border_td">{value.halfday_count}</td>
                                                <td className="border_td">{value.paid_leave_count}</td>
                                                <td className="border_td">{value.unmarked_count}</td>
                                            </tr>
                                                )
                                            })}
