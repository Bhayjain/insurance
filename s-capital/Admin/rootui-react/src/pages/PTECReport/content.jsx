 ...
 class Content extends Component {
    constructor( props ) {
        ...
        this.state = {
            ...
            loading:false,
            ptec_control : Cookies.get("genral_report_control")

        };
        this.ptec_report(this.state.sort_date)
    }

    ptec_report = (sort_date)=>  {
     ...
     console.log("params",params);
         const res = fetch(settings.api_url + "api/payroll/ptec_report", {
             method: 'POST',
             ...
             .then(json => {
                 console.log("Fetch PTEC ***************", json)
                 var data = json;
                 if (data.status == true) {
                     this.setState({
                      ptec_array: data.data,
                      isLoading:"none",
                      no_data_message:"none",
                     });
                 }
    ...
    export_ptec_report = ()=>  {
     ...
     console.log("Export params",params);
         const res = fetch(settings.api_url + "api/payroll/export_ptec_report", {
             method: 'POST',
             body: JSON.stringify(params),
             headers: {
                 "Content-type": "application/json; charset=UTF-8",
                 ...
                 <PageTitle className="payroll_new">
                      <div className="row">
                         <div className="col-lg-8 col-md-4 col-sm-12">
                             <h1 className="heading_top">PTEC Report</h1>
                         </div>
                      ...
                      <div className="col-lg-1 col-md-4 col-sm-12" style={{paddingLeft:"0px", textAlign:"end"}}>
                            <Button  disabled={this.state.ptec_control == "false" ? true : false} onClick={()=>this.export_ptec_report()}>Export{this.state.loading ? (
                                                                <Spinner />
                                                            ) : ''}</Button>
                         </div>
                                   ...
                                   <Table striped>
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col" className="border_td" style={{textAlign:"end"}}>PTEC Amount</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {this.state.ptec_array.map((value,index)=>{
                                                return(
                                            <tr key={index}>
                                                <th scope="row" className="border_td">
                                                    <div>{value.emp_name}</div>
                                                </th>
                                                <td className="border_td" style={{textAlign:"end"}}>&#x20b9;{(value.ptec_amount).toFixed(2)}</td>
                                            </tr>
                                                )
                                            })}
