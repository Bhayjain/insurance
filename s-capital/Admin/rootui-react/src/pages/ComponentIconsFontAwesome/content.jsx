 ...
 class Content extends Component {
     ...
     fetch_all_daily_sales_report = (startDate,endDate,pageNumber)=>  {
         ...
         const { settings } = this.props;
          const res = fetch(settings.api_url + "fetch_sm_business_wise_report", {
              method: 'POST',
              ...
              .then(json => {
                  ...
                  if (data.status == true) {
                      this.setState({
                          ...
                          total_no_of_policy: data.total_policy,
     ...
     daily_sales_report = ()=>  {
        ...
        //console.log("Export Params",params);
          const res = fetch(settings.api_url + "export_sm_business_wise_report", {
              method: 'POST',
              body: JSON.stringify(params),
              headers: {
                  "Content-type": "application/json; charset=UTF-8",
              ...
              return (
                  <div key={number} style={{
                      ...
                      style={{
                          ...
                          onClick={() => {
                              this.fetch_all_daily_sales_report(this.state.startDate,this.state.endDate,number)
                                  , this.setState({
                                      current_page: number,
                                      spinner_1: 'block'
                                  })
                      ...
                      style={{
                          ...
                          className={classes,"pagination_1"}
                          onClick={() => {
                              this.fetch_all_daily_sales_report(this.state.startDate,this.state.endDate,number + 1)
              ...
              <div className="mycalendar" style={{display: this.state.no_data=="none" ? "block" :"none",height:my_height - 70}}>
                 ...
                 <Table striped>
                         ...
                         <tbody>
                             {this.state.daily_sales_array.map((value,index)=>{
                              ...
                              <tr >
                                     ...
                                     <td className="table_sub_head total_new" style={{textAlign:"end"}}>{this.state.total_no_of_policy}</td>
                         ...
                         {/* <div style={{display:this.state.total_pages==1?"none":'inline-flex',width:"100%",marginTop:"10px",marginBottom:"20px",padding: "1px 8px"}}>
                              ...
                              outline onClick={() => this.fetch_all_daily_sales_report(this.state.startDate,this.state.endDate,1)}>first</Button>
                              ...
                              onClick={() => {
                                  if (this.state.current_page > 1) {
                                    this.fetch_all_daily_sales_report(this.state.startDate,this.state.endDate,this.state.current_page - 1)
                                  } else {
                                    this.fetch_all_daily_sales_report(this.state.startDate,this.state.endDate,this.state.current_page)
                                  }
                              }}
                              >Previous</Button>
                                {renderPageNumbers}
                              ...
                              outline onClick={() => this.fetch_all_daily_sales_report(this.state.startDate,this.state.endDate,this.state.total_pages)}>last</Button>
                            </div> */}
